"""
NKIBench: rope_single_freq_apply (SDK 2.28 namespace)
Original: NKIBench/kernels/rope_single_freq_apply_B1_H64_N4096_D128_0.py
Pattern: RoPE using first/second half split, tile along S dimension
Key learning: d_head on P dim, S on F dim, SBUF packing for cos/sin
"""
import nki
...
def kernel(x_in, cos, sin):
    ...
    x_in: [d_head, S], cos/sin: [d_head // 2, S]
    Uses first/second half split:
      result[:d_head/2] = x[:d_head/2] * cos - x[d_head/2:] * sin
      result[d_head/2:] = x[d_head/2:] * cos + x[:d_head/2] * sin
    """
    d_head, S = x_in.shape
    half_d = d_head // 2
    ...
    for i_S_offset in range(0, S, tile_size_S):
        ...
        x_out_sb[i_upper, i_S] = nisa.tensor_tensor(e_cos, o_sin, nl.subtract)
        x_out_sb[i_lower, i_S] = nisa.tensor_tensor(o_cos, e_sin, nl.add)
